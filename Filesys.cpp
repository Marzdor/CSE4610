...
Filesys::Filesys(std::string disk_name, int number_of_blocks, int block_size)
...
int Filesys::CreateNewFile(std::string filename)
{
...
int Filesys::RemoveFile(std::string filename)
{
  std::string formatted_filename = FormatteFileName(filename);
  int file_location = FindStringIndex(this->file_names,formatted_filename);
  bool file_exists = file_location != -1;
...
int Filesys::AddBlock(std::string filename, std::string buffer)
{
...
int Filesys::DeleteBlock(std::string filename, int block_number)
{
  std::string formatted_filename = FormatteFileName(filename);
  int file_location = FindStringIndex(this->file_names,formatted_filename);
  bool file_exists = file_location != -1;
...
int Filesys::ReadBlock(std::string filename, int block_number, std::string & buffer)
{
  std::string formatted_filename = FormatteFileName(filename);
...
int Filesys::WriteBlock(std::string filename, int block_number, std::string buffer)
{
  std::string formatted_filename = FormatteFileName(filename);
  int found_block = this->CheckBlock(formatted_filename,block_number);
...
void Filesys::InitializeFileSystem()
{
...
void Filesys::LoadRoot(bool fresh_drive)
{
  std::stringstream ss;
...
void Filesys::LoadFat(bool fresh_drive)
{
  std::stringstream ss;
...
int Filesys::CheckBlock(std::string filename, int block_number)
{
  std::string formatted_filename = FormatteFileName(filename);
